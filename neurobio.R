...
ab40i2_data <- abeta %>%
    select(-c(eid, plasma_abeta40_i3,
              plasma_abeta42_i2, plasma_abeta42_i3,
...
ab40i3_data <- abeta %>%
    select(-c(eid, plasma_abeta40_i2,
              plasma_abeta42_i2, plasma_abeta42_i3,
              e1, e2, e3, e4, rs429358, rs7412, Age2,
...
delta_ab40_data <- abeta %>%
    select(-c(eid,plasma_abeta42_i2, plasma_abeta42_i3,
              e1, e2, e3, e4, rs429358, rs7412, Age2)) %>%
...
delta_ab42_data <- abeta %>%
    select(-c(eid,plasma_abeta40_i2, plasma_abeta40_i3,
              e1, e2, e3, e4, rs429358, rs7412, Age2)) %>%
...
# NFL and Glialfap
nflgli <- covid %>%
    select(eid, plasma_nfl_i2, plasma_nfl_i3,
...
delta_nfl_data <- nflgli %>%
    select(-c(eid,plasma_glialfap_i2, plasma_glialfap_i3,
              e1, e2, e3, e4, rs429358, rs7412, Age2)) %>%
...
delta_gli_data <- nflgli %>%
    select(-c(eid,plasma_nfl_i2, plasma_nfl_i3,
              e1, e2, e3, e4, rs429358, rs7412, Age2)) %>%
...
covid_recovery <- covid[,c(1,24:30)] %>%
    ...
    filter(c19_count != "I do not know if I have had COVID-19",
...
longcov <- merge(covid_recovery, apoe) %>%
    merge(., covar) %>%
    select(-c(eid, first_c19_date, first_c19_diag, recent_c19_date,
